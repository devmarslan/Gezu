...
// Helper to get the country code from a currency code for flag display
function getCountryCodeFromCurrency(currencyCode) {
  // Common currency-to-country mappings where currency prefix doesn't match country
  ...
  return currencyToCountry[currencyCode] || currencyCode.slice(0, 2).toLowerCase();
}
...
function buildFromData(data) {
  ...
  if (moneyFormats[currencyCode] && currencyCode) {
    // Get proper country code for flag display
    const flagCode = getCountryCodeFromCurrency(currencyCode);
    ...
    $$4(selector).forEach(function (el) {
      if (el.querySelector('button[value="'+ currencyCode +'"]') == null) {
        ...
        if( el.querySelector('.hdt-item-auto img')) {
          el.querySelector('.hdt-item-auto img').setAttribute("src", "//cdn.shopify.com/static/images/flags/"+ flagCode +".svg");
...
document.addEventListener("richlist:change", (e) => {
  if (e.target.name === "country_code" && e.detail.value !== newCurrency) {
    ...
    // Use proper country code for flag display
    const flagCode = getCountryCodeFromCurrency(e.detail.value);
    if($4('[data-flag-current]')) {
      $$4('[data-flag-current]').forEach(function (el) {
        el.setAttribute("src", "//cdn.shopify.com/static/images/flags/"+ flagCode +".svg");
        el.setAttribute("srcset", "//cdn.shopify.com/static/images/flags/"+ flagCode +".svg?width=32 32w");
